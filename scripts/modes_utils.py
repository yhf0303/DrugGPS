...
class Attention(nn.Module):
    def __init__(self, config, vis, mm=True):
        ...
        if mm:
            self.query_text = nn.Linear(config.hidden_size, self.all_head_size)
            self.key_text = nn.Linear(config.hidden_size, self.all_head_size)
            self.value_text = nn.Linear(config.hidden_size, self.all_head_size)
            self.out_text = nn.Linear(config.hidden_size, config.hidden_size)
            self.attn_dropout_text = nn.Dropout(config.transformer["attention_dropout_rate"])
            self.attn_dropout_it = nn.Dropout(config.transformer["attention_dropout_rate"])
            self.attn_dropout_ti = nn.Dropout(config.transformer["attention_dropout_rate"])
            self.proj_dropout_text = nn.Dropout(config.transformer["attention_dropout_rate"])
